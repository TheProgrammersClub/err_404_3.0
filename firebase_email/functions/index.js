...
// const gmailEmail = encodeURIComponent("csi@mhssce.ac.in");
// const gmailPassword = encodeURIComponent("csi@2017");

const mailTransport = nodemailer.createTransport(
`smtps://${gmailEmail}:${gmailPassword}@smtp.gmail.com`);
// const mailTransport = nodemailer.createTransport(
// 	`smtps://"programmersClub":"Programmers@Club@123@$$"@smtp.sendgrid.net`);
...
// 	}
// });
exports.sendEmailCheckinConfirmation = functions.database.ref('/checkin/{pushId}').onWrite(event => {
	let pushId = event.params.pushId;
	let data = event.data.val();

	let send_to = data.email;
	...
	mailOptions.subject = "Err_404 | Seat Confirmed";
	...
	mailTransport.sendMail(mailOptions).then(() => {
		console.log('New entry: Email sent to:'+send_to);
	}).catch(error => {
		console.error('There was an error while sending the email to :', error, send_to, team_name);  
	});	
	...
	// };
	// sgMail.send(msg);
	// console.log("Email Set to: "+send_to);
});